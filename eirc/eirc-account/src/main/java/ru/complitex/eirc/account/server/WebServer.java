...
public class WebServer {
    public static void main(String[] args) throws IOException {
        ...
        try {
            properties.load(new FileInputStream("eirc-account.properties"));
        } catch (IOException e) {
            System.out.println("eirc-account.properties not found");
        }

        String host = properties.getProperty("host", "localhost");
        int port = Integer.parseInt(properties.getProperty("port", "7778"));
        ...
        WebSocketEngine.getEngine().register("", "/wicket/websocket", new WebSocketApplication() {
            ...
            public WebSocket createSocket(ProtocolHandler handler, HttpRequestPacket requestPacket, WebSocketListener... listeners) {
                return new WebSocket(handler, requestPacket, listeners);
            }
        });
