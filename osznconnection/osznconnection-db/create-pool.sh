...
$GLASSFISH_ASADMIN create-auth-realm --classname="com.sun.enterprise.security.auth.realm.jdbc.JDBCRealm" --property="jaas-context=jdbcRealm:datasource-jndi=jdbc/osznconnectionResource:user-table=user:user-name-column=login:password-column=password:group-table=usergroup:group-name-column=group_name:charset=UTF-8" osznconnectionRealm
