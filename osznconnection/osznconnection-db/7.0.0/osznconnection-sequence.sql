...
update sequence set sequence_value = (select IFNULL(max(`id`), 0) from `ownership_string_value`)+1 where sequence_name = 'ownership_string_value';
update sequence set sequence_value = (select IFNULL(max(`object_id`), 0) from `ownership`)+1 where sequence_name = 'ownership';
update sequence set sequence_value = (select IFNULL(max(`id`), 0) from `privilege_string_value`)+1 where sequence_name = 'privilege_string_value';
