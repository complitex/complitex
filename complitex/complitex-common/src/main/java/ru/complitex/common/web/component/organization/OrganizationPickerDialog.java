...
public class OrganizationPickerDialog extends Panel {
    ...
    public OrganizationPickerDialog(String id, IModel<DomainObject> organizationModel, Long... organizationTypeIds) {
        ...
        example.addAttributeFilter(new AttributeFilter(IOrganizationStrategy.NAME));
        example.addAttributeFilter(new AttributeFilter(IOrganizationStrategy.CODE));
        example.setLocaleId(Locales.getLocaleId(getLocale()));

        if (organizationTypeIds != null && organizationTypeIds.length > 0) {
            example.addAdditionalParam(IOrganizationStrategy.ORGANIZATION_TYPE_PARAMETER, organizationTypeIds);
        ...
        form.add(new TextField<>("nameFilter", new AttributeExampleModel(example, IOrganizationStrategy.NAME))
        ...
        form.add(new TextField<>("codeFilter", new AttributeExampleModel(example, IOrganizationStrategy.CODE))
                .add(new OnChangeAjaxBehavior() {
                    @Override
                    protected void onUpdate(AjaxRequestTarget target) {
                    }
        ...
        DataView<DomainObject> data = new DataView<DomainObject>("data", dataProvider) {
            ...
            protected void populateItem(Item<DomainObject> item) {
                ...
                item.add(new Label("name", organization.getStringValue(IOrganizationStrategy.NAME, getLocale())));
